...
A single command to "bootstrap" a new system to pull down all of my dotfiles and config's, as well as install the tools I commonly use. 

In addition, `dotfiles` may be re-executed at any time to synchronize anything that might have changed.
...
1. In Linux, Git is installed if necessary via the native package manager (it's already there in OSX).
