...
local notify = require("hs.notify") -- http://www.hammerspoon.org/docs/hs.settings.html
...
-- modules should export stuff
local export = {}

local nothingNotification = notify.new({
	title = 'Pasteboard',
	informativeText = 'Nothing to paste',
})

function pasteSelection(selected)
	targetApp:focus()
	if not selected then return end

	paste(selected.text, {})
	hs.printf(selected.text)
end

function paste(string, key)
	if (pasteOnSelect or key.alt == true) then
...
function storeCopy()
	...
	if (now > last_change) then
		...
		current_clipboard = {
			text = pasteboard.getContents(),
			app = app:name(),
			bundleID = app:bundleID(),
		}
		-- asmagill requested this feature. It prevents the history from keeping items removed by password managers
		if (current_clipboard.text == nil and honor_clearcontent) then
			clearLastItem()
		elseif alreadyExists(current_clipboard) then
			return
		else
...
function alreadyExists(item)
	local hash = hs.hash.MD5(item.text)

	for k,v in pairs(clipboard_history) do
		if (hash == hs.hash.MD5(v.text)) then
			hs.printf("already have " .. item.text)
			return true
		end
	end
...
populateMenu = function(key)
	...
	for k,v in pairs(clipboard_history) do
		table.insert(menuData, 1, {
			text = v.text,
			subText = v.app,
			image = hs.image.imageFromAppBundle(v.bundleID),
		})
	end
...
function sync()
	hs.printf('pasteboard sync')
	settings.set("so.victor.hs.jumpcut", clipboard_history)
	now = pasteboard.changeCount()
	export.chooser:refreshChoicesCallback()
end

function export.clear()
	pasteboard.clearContents()
	clipboard_history = {}
-- this change will get picked up by the timer
--	sync()
end

chooser = hs.chooser.new(pasteSelection)
export.chooser = chooser
export.sync = sync

chooser:choices(populateMenu)

function export.show()
	targetApp = hs.window.focusedWindow()
	local count = #clipboard_history

	if count == 0 then
		nothingNotification:send()
	else
		if count < 10 then
			chooser:rows(count)
		else
